...
func Example_ReadRequest_GET() {
	...
	//Output:
	//test: NewRequest(file://[cwd]/test/get-request.txt) -> [status:OK] [ctx:context.Background] [content-location:github/advanced-go/example-domain/activity/EntryV1]
	//test: NewRequest(file://[cwd]/test/get-request.txt) -> [status:OK] [ctx:context.Background] [content-location:github/advanced-go/example-domain/activity/EntryV1]

}

func Example_ReadRequest_Baseline() {
	s := "file://[cwd]/test/baseline-request.txt"
	req, err := NewRequest(parseRaw(s))
	...
	//Output:
	//test: NewRequest(file://[cwd]/test/baseline-request.txt) -> [status:OK]
...
func Example_ReadRequest_PUT() {
	...
	//Output:
	//test: NewRequest(file://[cwd]/test/put-req.txt) -> [cnt:2] [fields:[{ingress 800µs usa west  access-log https://access-log.com/example-domain/timeseries/entry http access-log.com /example-domain/timeseries/entry GET 200} {egress 100µs usa east  access-log https://access-log.com/example-domain/timeseries/entry http access-log.com /example-domain/timeseries/entry PUT 202}]]

}

func ExampleNewRequest_Overrides() {
	s := "file://[cwd]/test/get-request-overrides.txt"
	req, err := NewRequest(parseRaw(s))

	fmt.Printf("test: NewRequest(%v) -> [status:%v] [header:%v]\n", s, err, req.Header[httpx.ContentResolver])

	//Output:
	//test: NewRequest(file://[cwd]/test/get-request-overrides.txt) -> [status:OK] [header:[github/advanced-go/observation:v1/timeseries/egress/entry?region=*->file:///f:/test/info.json github/advanced-go/observation:v1/timeseries/egress/entry?region=*->file:///f:/test/test.json]]
...
func ExampleNewRequest_Overrides_Empty() {
	...
	//Output:
	//test: NewRequest(file://[cwd]/test/get-request-overrides.txt) -> [err:OK] [ok:true] [str:1]
